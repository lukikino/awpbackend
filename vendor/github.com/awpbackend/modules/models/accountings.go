...
type AccountingsWithSummary struct {
	Accountings []Accountings `json:"accountings"`
	Summary     Accountings   `json:"summary"`
}
...
func GetAccountings(loginID int, search AccountingSearch) ReturnData {
	db := GetConnection()
	t := []Accountings{}
	err := db.Select(&t, "call sp_getAccountings(?,?,?,?,?,?,?,?)", loginID, search.Users, search.Stores, search.Machines, search.StartTime, search.EndTime, search.GroupBy, search.TimeRange)
	r := AccountingsWithSummary{}
	if err == nil {
		r.Accountings = t
		for _, item := range t {
